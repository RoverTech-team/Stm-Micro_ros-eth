/**
 * CM4 bare-metal firmware — CmdFollow build.
 *
...
int main(void)
{
  ...
  /* Give the 24 V rail and powerSTEP01 drivers ~2 s to stabilize
   * before the first fault poll, otherwise transient UVLO/OCD
   * during power-on latches faults and joints are skipped forever. */
  DelayMs(2000);
...
static void Motor_ProcessCommands(void)
{
  ...
  if (current_seq == last_processed_cmd_seq) {
    ...
    for (uint8_t i = 0; i < N_JOINTS; i++) {
      if (i != PHYS_J2 && i != PHYS_J3) continue;
      ...
      if ((now_ms - motion_pending_start_ms[i]) >= BRAKE_SETTLE_MS) {
        if (i == PHYS_J2)       RARM_EngageBrake(J2_INDEX);
        else if (i == PHYS_J3)  RARM_EngageBrake(J3_INDEX);
