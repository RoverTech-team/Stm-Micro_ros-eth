...
/**
 * TODO: Configure system clock for CM4.
 * The CM7 core typically configures the main clocks.
 * CM4 may need to enable its own peripheral clocks.
 */
static void SystemClock_Config(void)
{
    /* CM7 handles main PLL configuration.
     * CM4 just needs to enable the clocks for its peripherals. */
}
...
static void MX_GPIO_Init(void)
{
    /* Enable GPIOB (LEDs) and GPIOD (ultrasonic) clocks */
    ...
    /* DRV_CS pin for powerSTEP01 chip-select (output, start high/deasserted)
     * TODO: Verify this matches actual hardware wiring! */
    RCC->AHB4ENR |= RCC_AHB4ENR_GPIOAEN;
    BUSY(100);
    GPIO_InitTypeDef gpio_cs = {0};
    gpio_cs.Pin = DRV_CS_Pin;
    gpio_cs.Mode = GPIO_MODE_OUTPUT_PP;
    gpio_cs.Pull = GPIO_NOPULL;
    gpio_cs.Speed = GPIO_SPEED_FREQ_HIGH;
    HAL_GPIO_Init(DRV_CS_GPIO_Port, &gpio_cs);
...
int main(void)
{
    ...
    /*
     * TODO: Configure each motor with actual parameters from electronics team.
     * You need an RARM_SimpleConfig_t for each joint with correct values for:
     *   - steps_rev, reduction_ratio
     *   - supply_voltage, run_acc_dec_voltage, hold_voltage
     *   - max_speed, min_speed, acceleration, deceleration
     *   - oc_threshold, stall_threshold
     *   - min_degs, max_degs
     *
     * Example (placeholder values — REPLACE WITH REAL ONES):
     *
     * RARM_SimpleConfig_t joint0_config = {
     *     .OVERCURRENT_SD = OC_SHUTDOWN,
     *     .VSCOMP = VSCOMP_DISABLE,
     *     .STEP_MODE = SM_16_MICROSTEP,
     *     .steps_rev = 200,
     *     .reduction_ratio = 1,
     ...
     *     .max_degs = 180,
     * };
     * RARM_SetConfig(J1_INDEX, &joint0_config);
     * RARM_SetConfig(J2_INDEX, &joint0_config);  // or different config
     */
